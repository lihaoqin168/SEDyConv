...
class BinaryDiceLoss(nn.Module):
    ...
    def forward(self, predict, target):
        assert predict.shape[0] == target.shape[0], "predict_manual & target batch size don't match"
...
class CELoss4MOTS(nn.Module):
    ...
    def forward(self, predict, target):
        assert predict.shape == target.shape, 'predict_manual & target shape do not match'
