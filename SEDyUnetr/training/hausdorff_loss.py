...
class HausdorffDTLoss(nn.Module):
    ...
    def forward(
        ...
        pred_np = copy.deepcopy(pred.cpu().detach().numpy())
        target_np = copy.deepcopy(target.cpu().detach().numpy())
        pred_dt = torch.from_numpy(self.distance_field(pred_np)).float()
        target_dt = torch.from_numpy(self.distance_field(target_np)).float()
