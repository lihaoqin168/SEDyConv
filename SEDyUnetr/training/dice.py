...
class DiceLoss(_Loss):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        # print("+ dice input shape ", input.shape)
...
class DiceCELoss(_Loss):
    ...
    def __init__(
        ...
        #     ignore_index = 0
        print("+ include_background", include_background)
        print("+ CrossEntropyLoss ignore_index", ignore_index)
        self.cross_entropy = nn.CrossEntropyLoss(ignore_index=ignore_index, weight=ce_weight, reduction=reduction)
        if lambda_dice < 0.0:
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        if torch.isnan(ce_loss).any():
            total_loss: torch.Tensor = dice_loss
        else:
