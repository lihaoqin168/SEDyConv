...
def sliding_window_inference(
    ...
    **kwargs: Any,
    ...
    Args:
        inputs: input of model. inputs[0]: image to be processed (assuming NCHW[D])
        ...
        sigma_scale: the standard deviation coefficient_vary of the Gaussian window when `mode` is ``"gaussian"``.
    ...
    for slice_g in range(0, total_slices, sw_batch_size):
        ...
        if other_inputs == None:
            ...
            seg_prob = predictor(tuple([window_data]) + other_inputs, *args, **kwargs).to(device) #[1,14,96,96,96] # batched patch segmentation

        if not _initialized:  # init. buffer at the first iteration
            output_classes = seg_prob.shape[1]#14
            output_shape = [batch_size, output_classes] + list(image_size)
            # allocate memory to store the full output and the count for overlapping parts
            output_image = torch.zeros(output_shape, dtype=torch.float32, device=device)#[1,14,306,240,180]
