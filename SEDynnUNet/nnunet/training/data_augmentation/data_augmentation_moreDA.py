...
def get_moreDA_augmentation_classify(dataloader_train, dataloader_val, patch_size, params=default_3D_augmentation_params,
                            border_val_seg=-1,
...
def get_moreDA_augmentation(dataloader_train, dataloader_val, patch_size, params=default_3D_augmentation_params,
    ...
    if use_nondetMultiThreadedAugmenter:
        ...
        batchgenerator_train = MultiThreadedAugmenter(dataloader_train, tr_transforms, params.get('num_threads'),
                                                      params.get("num_cached_per_thread"),
                                                      seeds=seeds_train, pin_memory=pin_memory)
        print("++num_threads", params.get('num_threads'))
        print("++num_cached_per_thread", params.get("num_cached_per_thread"))
        print("++pin_memory_queue", max(3, params.get("num_cached_per_thread") * params.get('num_threads') // 2))
    # batchgenerator_train = SingleThreadedAugmenter(dataloader_train, tr_transforms)
    # import IPython;IPython.embed()
