...
def load_dataset(folder, num_cases_properties_loading_threshold=1000):
    ...
    if len(case_identifiers) <= num_cases_properties_loading_threshold:
        ...
        for i in dataset.keys():
            dataset[i]['properties'] = load_pickle(dataset[i]['properties_file'])#haoqin load train and label path from 'imagesTr' 'labelsTr' in raw dir

    return dataset

def load_dataset_classify(folder, num_cases_properties_loading_threshold=1000):
    # we don't load the actual data but instead return the filename to the np file.
    print('loading dataset')
    # case_identifiers = get_case_identifiers(folder)
    # case_identifiers.sort()
...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        case_properties = []
        # print("\n haoqin selected_keys:"+str(selected_keys))
        ...
        for j, i in enumerate(selected_keys):
            ...
            if not force_fg:
                ...
                if len(foreground_classes) == 0:
                    ...
                    print('1 case does not contain any foreground classes', i)
                else:
...
class DataLoader2D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):# ！！！！haoqin
        selected_keys = np.random.choice(self.list_of_keys, self.batch_size, True, None)
        ...
        for j, i in enumerate(selected_keys):
            ...
            if not force_fg:
                ...
                if len(foreground_classes) == 0:
                    selected_class = None
                    random_slice = np.random.choice(case_all_data.shape[1])
                    print('2 case does not contain any foreground classes', i)
                else:
                    selected_class = np.random.choice(foreground_classes)
