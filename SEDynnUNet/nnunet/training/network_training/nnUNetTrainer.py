...
from nnunet.training.dataloading.dataset_loading import load_dataset, load_dataset_classify, DataLoader3D, DataLoader2D, unpack_dataset
...
class nnUNetTrainer(NetworkTrainer):
    ...
    def run_class_training(self):
        self.save_debug_information()
        super(nnUNetTrainer, self).run_class_training()
    ...
    def load_dataset_classify(self):
        self.dataset = load_dataset_classify(self.folder_with_preprocessed_data)

    def get_basic_generators(self):
        self.print_to_log_file("\n haoqin 333 load_dataset")
        self.load_dataset()#value dataset
        self.do_split()#value dataset_tr dataset_val
    ...
    def get_basic_generators_4classify(self):
        from torch.utils.data import DataLoader
        self.print_to_log_file("\n haoqin 444 classify load_dataset")
        self.load_dataset_classify()#value self.dataset
        self.do_split_classify()#random train and val, value self.dataset_tr, self.dataset_val
        ...
        if self.threeD:
            ...
            #                       oversample_foreground_percent=self.oversample_foreground_percent,
            #                       pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
            dl_tr = DataLoader(dataset=self.dataset_tr, batch_size=self.batch_size)
            dl_val = DataLoader(dataset=self.dataset_val, batch_size=self.batch_size)
        return dl_tr, dl_val

    def preprocess_patient(self, input_files):
        """
        Used to predict_manual new unseen data. Not used for the preprocessing of the training/test data
        :param input_files:
        :return:
        """
    ...
    def validate(self, do_mirroring: bool = True, use_sliding_window: bool = True, step_size: float = 0.5,
        ...
        if do_mirroring:
            if not self.data_aug_params['do_mirror']:
                ...
                print("We did not train with mirroring so you cannot do inference with mirroring enabled")
                mirror_axes = ()
    ...
    def finish_online_evaluation(self):
        ...
        self.print_to_log_file("haoqin Average global foreground Dice:", [np.round(i, 4) for i in global_dc_per_class],"mean:", np.mean(global_dc_per_class))
