...
class NetworkTrainer(object):
    ...
    @abstractmethod
    def initialize_classify(self, training=True):
        """
        for classify
        different: val_gen
        :param training:
    ...
    def get_dataset_tr_val_fromJson(self, jsonfile):
        pass
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        ...
        self.print_to_log_file("saving checkpoint...", fname)
        save_this = {
            'epoch': self.epoch + 1,
    ...
    def load_checkpoint(self, fname, train=True):
        ...
        # saved_model = tensor.load(fname, map_location=tensor.device('cuda', tensor.cuda.current_device()))
    ...
    def load_checkpoint_ram(self, checkpoint, train=True):
        ...
        if train:
            ...
            if self.lr_scheduler is not None and issubclass(self.lr_scheduler.__class__, self.lr_scheduler._LRScheduler):
    ...
    def run_training(self):
        if not torch.cuda.is_available():
            self.print_to_log_file("WARNING!!! You are attempting to run training on a CPU (tensor.cuda.is_available() is False). This can be VERY slow!")

        start_time = time()
        _ = self.tr_gen.next()
        print("self.tr_gen.next()  time {:.2f}s".format(time() - start_time))
        start_time = time()
        _ = self.val_gen.next()
        print("self.val_gen.next()  time {:.2f}s".format(time() - start_time))
        ...
        if cudnn.benchmark and cudnn.deterministic:
            warn("tensor.backends.cudnn.deterministic is True indicating a deterministic training is desired. "
                 "But tensor.backends.cudnn.benchmark is True as well and this will prevent deterministic training! "
        ...
        while self.epoch < self.max_num_epochs:
            ...
            if self.use_progress_bar:
                ...
                self.print_to_log_file("haoqin 111")
                self.print_to_log_file("num_batches_per_epoch", self.num_batches_per_epoch)
                for idx in range(self.num_batches_per_epoch):
                    start_time = time()
                    l = self.run_iteration(self.tr_gen, True, idx=idx)#train loss
                    # print("num_batches_per_epoch run_iteration time {:.2f}s".format(time() - start_time))
                    # self.print_to_log_file("run_iteration, loss=", l)
                    train_losses_epoch.append(l)

            self.all_tr_losses.append(np.mean(train_losses_epoch))
            self.print_to_log_file("haoqin train loss : %.4f" % self.all_tr_losses[-1])
            ...
            with torch.no_grad():
                ...
                for b in range(self.num_val_batches_per_epoch):
                    l = self.run_iteration(self.val_gen, False, True)#validation loss
                    # self.print_to_log_file("run_iteration, validation loss=", l)
                    val_losses.append(l)
                self.all_val_losses.append(np.mean(val_losses))
                self.print_to_log_file("validation loss(train=False): %.4f" % self.all_val_losses[-1])
                ...
                if self.also_val_in_tr_mode:
                    ...
                    self.print_to_log_file("222 validation loss (train=True): %.4f" % self.all_val_losses_tr_mode[-1])
    ...
    def update_eval_criterion_MA(self):
        ...
        self.print_to_log_file("update val_eval_criterion_MA ", self.val_eval_criterion_MA)
