...
from nnunet.inference.segmentation_export_CAM import save_segmentation_nifti_from_softmax, save_segmentation_nifti
...
# from nnunet.medcam import medcam
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
                  ...
                  segmentation_export_kwargs: dict = None, disable_postprocessing: bool = False, cam_lable=1, cam_layer='auto', output_channel="default"):
    """
    ...
    for name, m in trainer.network.named_modules():
        if isinstance(m, torch.nn.Conv3d):
            print(name)
    #M3D CAM
    """Herby the parameters of `medcam.inject` have the following meaning: \\
    `label=2`: nnUNet encodes the different classes it is able to predict_manual/segment in the channel dimension. The label keyword tells medcam that we are only interested in the attention maps for the third class. \\
    ...
    print("++ cam_lable", cam_lable)
    print("++ cam_layer", cam_layer)
    # trainer.network = medcam.inject(trainer.network, label=cam_lable, replace=True, backend="gcampp", layer='seg_outputs.4')
    trainer.network = medcam.inject(trainer.network, label=cam_lable, replace=True, backend="gcampp", layer=cam_layer)
    # trainer.network = medcam.inject(trainer.network, label=cam_lable, replace=True, backend="gcampp", layer=cam_layer, channels=output_channel)
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
                        ...
                        segmentation_export_kwargs: dict = None, disable_postprocessing: bool = False,
                        cam_lable=1, cam_layer='auto', output_channel='default'):
    """
    ...
    if mode == "normal":
        ...
        return predict_cases(model, list_of_lists[part_id::num_parts], output_files[part_id::num_parts], folds,
                             ...
                             disable_postprocessing=disable_postprocessing, cam_lable=cam_lable, cam_layer=cam_layer, output_channel=output_channel)
    # elif mode == "fast":
    #     if overwrite_all_in_gpu is None:
    #         all_in_gpu = False
    #     else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--part_id", type=int, required=False, default=0, help="Used to parallelize the prediction of "
                                                                               ...
                                                                               "want to use n GPUs to predict_manual this "
    ...
    parser.add_argument("--cam_lable", type=int, required=False, default=1, help="A class label of interest.")
    parser.add_argument("--cam_layer", type=str, required=False, default="auto", help="One or multiple layer names of the model from which attention maps will be extracted.")
    parser.add_argument("--output_channel", type=int, required=True, default=2,
                        help="the channel number of attention maps.")
    ...
    predict_from_folder(model, input_folder, output_folder, folds, save_npz, num_threads_preprocessing,
                        ...
                        cam_lable=args.cam_lable, cam_layer=args.cam_layer, output_channel=args.output_channel)
