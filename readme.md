...
<b>Fig. 1.</b>b> Schematic of the convolution space on which our spatially enhanced multi-dimensional dynamic convolution (SEDyConv) operates. For clarity, consider the 2D convolution kernel and ignore the bias. (a) The output channel dimension; that is, the number of filter dimensions, (b) the input channel dimension, (c) the 2D spatial dimension of all filters, and (d) the 3D kernel space dimension of one filter. We first compute four type weights , ,  and  for the original kernel . Then, we dynamically combine the processed  with switches to generate the final . During this process, weights  and  are used to preserve the spatial properties and expand the convolutional kernel space into N dimensions. The formulations and implementations are clarified in the Methodology section.
